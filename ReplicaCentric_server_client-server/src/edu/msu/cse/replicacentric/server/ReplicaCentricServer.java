...
public class ReplicaCentricServer extends DKVFServer {
    ...
    public ReplicaCentricServer(ConfigReader cnfReader) {
        ...
        for (int i = 1; i <= numOfServers; i++) {
            if (!protocolProperties.get("client" + i).isEmpty()) {
                List<String> replicas = protocolProperties.get("client" + i);
                for (int j = 0; j < replicas.size(); j++) {
                    for (int k = j + 1; k < replicas.size(); k++) {
                        // Add two directed edges between these two replicas.
                        if (!replicas.get(j).equals(replicas.get(k))) {
                            augmentedEdges.add(Edge.newBuilder().setVertex1(new Integer(replicas.get(j))).setVertex2(new Integer(replicas.get(k))).build());
                            augmentedEdges.add(Edge.newBuilder().setVertex1(new Integer(replicas.get(k))).setVertex2(new Integer(replicas.get(j))).build());
                        }
                    }
                }
            }
    ...
    private boolean handleMessagesNow(List<Dependency> timestampList) {
        // Firstly, check whether this get message should be handled now or not.
